...
# Optional: verify folders early
print("TRAIN_DIR:", os.path.abspath(TRAIN_DIR))
print("TEST_DIR: ", os.path.abspath(TEST_DIR))
assert os.path.isdir(TRAIN_DIR), f"TRAIN_DIR not found: {os.path.abspath(TRAIN_DIR)}"
assert os.path.isdir(TEST_DIR),  f"TEST_DIR not found:  {os.path.abspath(TEST_DIR)}"

# ======================
# Safe librosa wrappers (handle version differences)
# ======================
def safe_time_stretch(y: np.ndarray, rate: float) -> np.ndarray:
    try:
        sig = inspect.signature(librosa.effects.time_stretch)
        # Prefer keyword form
        return librosa.effects.time_stretch(y=y, rate=rate)
    except Exception:
        return y

def safe_pitch_shift(y: np.ndarray, sr: int, n_steps: float) -> np.ndarray:
    try:
        # Prefer keyword form
...
def augment_audio(audio, sr):
    choice = random.choice(["none", "noise", "pitch", "speed", "shift", "reverb"])
    if choice == "noise":
        audio = audio + np.random.normal(0, 0.005, size=audio.shape)
    elif choice == "pitch":
        n_steps = random.uniform(-3, 3)
        audio = safe_pitch_shift(audio, sr, n_steps)
    elif choice == "speed":
        rate = random.uniform(0.9, 1.1)
        audio = safe_time_stretch(audio, rate)
    elif choice == "shift":
        shift = int(random.uniform(-0.2, 0.2) * sr)
        audio = np.roll(audio, shift)
    elif choice == "reverb":
        decay = random.uniform(0.1, 0.3)
        audio = np.convolve(audio, np.exp(-decay * np.arange(0, sr // 10)))[: len(audio)]
    return audio
